<template>
    <div class="w-full h-full lg:h-screen">
        <h1 class="ml-40 mt-10 mb-8 text-5xl">Atualizar Dados</h1>
        <form v-if="!isLoading" @keydown="validarCampo" class="w-4/5 mx-auto border border-neutral rounded-2xl py-4 px-8"
            ...
            <Butoon type="submit" @click="updateForm" class="btn btn-lg btn-primary mt-12">Atualizar</Butoon>
        </form>
        <form v-else class="w-4/5 mx-auto border border-success rounded-2xl py-4 px-8">
            <div class="w-full flex justify-center h-96">
                <h2 class="text-center text-success text-7xl my-auto">Atualizado com sucesso</h2>
            </div>
        </form>
    </div>
</template>
...
import { ref } from 'vue';
import { updateAssociate, getAssociateById, uploadAssociatePhoto } from '../api/associatesApi.ts'
    ...
    methods: {
        async getAssociate() {
            try {
                ...
                console.log('Associado encontrado:', response);
                this.form = response;
            } catch (error) {
        ...
        async updateForm() {
            try {
                const id = this.$route.params.id;
                this.isLoading = true;
                const response = await updateAssociate(id, this.form);
